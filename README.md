...
\`\`\`
All randomness derives from a deterministic PRNG seeded by `combinedSeed`. Verifier uses the **exact same PRNG order**: peg map → row decisions.  
**Peg Bias Logic**  
...
- **Rows:** 12 **Bins:** 13  
- Each row → Left/Right based on PRNG vs bias′  
...
## ✅ Verifier Page
Route: **`/verify`**  
Enter `serverSeed`, `clientSeed`, `nonce`, `dropColumn` to reproduce results. Displays ✅ if recomputed `commitHex`, `combinedSeed`, and `binIndex` match the stored round.
---
